...
{
  return trans ? trans[(to_uchar (c))] : c;
}
  ...
  while (len--)
    {
      ...
      struct tree *kwset_link = trie->links;
      struct tree *links[(8 + 8/2)];
      enum { L, R } dirs[(8 + 8/2)];
      links[0] = (struct tree *) &trie->links;
      dirs[0] = L;
      int depth = 1;

      while (kwset_link && label != kwset_link->label)
        {
          links[depth] = kwset_link;
          if (label < kwset_link->label)
            dirs[depth++] = L, kwset_link = kwset_link->llink;
          else
            dirs[depth++] = R, kwset_link = kwset_link->rlink;
        }
      ...
      if (!kwset_link)
        {
          kwset_link = __extension__ ({ struct obstack *__h = (&kwset->obstack); __extension__ ({ struct obstack *__o = (__h); size_t __len = ((sizeof *kwset_link)); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < __len) _obstack_newchunk (__o, __len); ((void) ((__o)->next_free += (__len))); }); __extension__ ({ struct obstack *__o1 = (__h); void *__value = (void *) __o1->object_base; if (__o1->next_free == __value) __o1->maybe_empty_object = 1; __o1->next_free = ((sizeof (ptrdiff_t) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (((__o1->next_free) - (sizeof (ptrdiff_t) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (__o1->alignment_mask)) & ~(__o1->alignment_mask))); if ((size_t) (__o1->next_free - (char *) __o1->chunk) > (size_t) (__o1->chunk_limit - (char *) __o1->chunk)) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; __value; }); });
          kwset_link->llink = 
# 193 "kwset.c" 3 4
                       ((void *)0)
# 193 "kwset.c"
...
# 194 "kwset.c"
                           ;
          kwset_link->trie = __extension__ ({ struct obstack *__h = (&kwset->obstack); __extension__ ({ struct obstack *__o = (__h); size_t __len = ((sizeof *kwset_link->trie)); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < __len) _obstack_newchunk (__o, __len); ((void) ((__o)->next_free += (__len))); }); __extension__ ({ struct obstack *__o1 = (__h); void *__value = (void *) __o1->object_base; if (__o1->next_free == __value) __o1->maybe_empty_object = 1; __o1->next_free = ((sizeof (ptrdiff_t) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (((__o1->next_free) - (sizeof (ptrdiff_t) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (__o1->alignment_mask)) & ~(__o1->alignment_mask))); if ((size_t) (__o1->next_free - (char *) __o1->chunk) > (size_t) (__o1->chunk_limit - (char *) __o1->chunk)) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; __value; }); });
          kwset_link->trie->accepting = 0;
...
# 197 "kwset.c"
          ...
          kwset_link->trie->next = 
# 199 "kwset.c" 3 4
                            ((void *)0)
# 199 "kwset.c"
                                ;
          kwset_link->trie->fail = 
# 200 "kwset.c" 3 4
                            ((void *)0)
# 200 "kwset.c"
                                ;
          ...
          kwset_link->label = label;
          kwset_link->balance = 0;


          if (dirs[--depth] == L)
            links[depth]->llink = kwset_link;
          else
            links[depth]->rlink = kwset_link;
           ...
           struct trie *recourse)
{
  ...
  while (fail)
    {
      kwset_link = fail->links;
      while (kwset_link && tree->label != kwset_link->label)
        if (tree->label < kwset_link->label)
...
# 459 "kwset.c"
                   + 1); ++i)
      kwset->next[i] = next[(to_uchar (trans[i]))];
  ...
  if (kwset->words == 1)
    {
      ...
      if (trans)
        {
          char const *equiv1 = memchr (trans, gc1, (
# 494 "kwset.c" 3 4
                                                  (0x7f * 2 + 1) 
# 494 "kwset.c"
                                                  + 1));
...
# 496 "kwset.c"
                                              + 1) - (equiv1 + 1));
          if (equiv2)
            gc1help = (memchr (equiv2 + 1, gc1, trans + (
# 498 "kwset.c" 3 4
                                                       (0x7f * 2 + 1) 
# 498 "kwset.c"
                                                       + 1) - (equiv2 + 1))
...
# 499 "kwset.c"
                        + 1)
                       : (to_uchar (gc1)) ^ (equiv1 - trans) ^ (equiv2 - trans));
        }
...
# 511 "kwset.c"
                   + 1); ++i)
      kwset->delta[i] = delta[(to_uchar (trans[i]))];
}
# 523 "kwset.c"
static inline 
# 523 "kwset.c" 3 4
...
# 578 "kwset.c"
  ...
  for (; s < slim; s++)
    if (kwset->trans[(to_uchar (*s))] == kwset->gc1)
      return s;
  n -= ntrans;
  return n == 0 ? 
# 584 "kwset.c" 3 4
                 ((void *)0) 
# 584 "kwset.c"
                      : memchr2 (s, kwset->gc1, kwset->gc1help, n);
}
...
static size_t __attribute__ ((__nonnull__ (4)))
